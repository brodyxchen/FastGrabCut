...
GMM::GMM()
{
	...
	_model.create( 1, modelSize*componentsCount, CV_64FC1 );	//CV_64F=64位float型，C1=一通道
	_model.setTo(Scalar(0));	//把矩阵初始化全0
    ...
    for( int ci = 0; ci < componentsCount; ci++ )
        if( coefs[ci] > 0 )	//假设用户给出了混合模型的所有参数，则计算对应的矩阵和行列式，且后面不在学习模型（initGMMs）
			 //计算GMM中第ci个高斯模型的协方差的逆Inverse和行列式Determinant
			 //为了后面计算每个像素属于该高斯模型的概率（也就是数据能量项）
             calcInverseCovAndDeterm( ci ); 
}

//背景和前景各有一个对应的GMM（混合高斯模型） //_model=1*(13*5)  即1行65列的矩阵 保存参数用
GMM::GMM( Mat& _model )
{
...
double GMM::operator()( int ci, const Vec3d color ) const
{
    ...
    if( coefs[ci] > 0 )
    {
		...
		//计算diff与inverseCovs[ci]两个矩阵的乘积（矩阵相乘）
        double mult = diff[0]*(diff[0]*inverseCovs[ci][0][0] + diff[1]*inverseCovs[ci][1][0] + diff[2]*inverseCovs[ci][2][0])
                   + diff[1]*(diff[0]*inverseCovs[ci][0][1] + diff[1]*inverseCovs[ci][1][1] + diff[2]*inverseCovs[ci][2][1])
                   + diff[2]*(diff[0]*inverseCovs[ci][0][2] + diff[1]*inverseCovs[ci][1][2] + diff[2]*inverseCovs[ci][2][2]);
        res = 1.0f/sqrt(covDeterms[ci]) * exp(-0.5f*mult);//按照高斯混合模型概率公式，这里的分母应该是 sqrt( pow(2Pi,3) * covDeterms[ci] )   3为像素点维数
    }
    return res;		//像素属于ci个高斯/正态模型的概率
}
...
//计计算这个高斯模型的参数:正态分布的均值方差协方差等）增加样本像素。计算加入color这个像素后，像素集
//中所有像素的RGB三个通道的和sums（用来计算均值），还有它的prods（用来计算协方差），
//并且记录这个像素集的像素个数和总的像素个数（用来计算这个高斯模型的权值）。
void GMM::addSample( int ci, const Vec3d color )			//为第ci个模型添加样本像素
{
    sums[ci][0] += color[0]; sums[ci][1] += color[1]; sums[ci][2] += color[2];
	// （0,1,2）^T * （0,1,2）  两个矩阵相乘
...
void GMM::endLearning()
{
    ...
    for( int ci = 0; ci < componentsCount; ci++ )
    {
        ...
        else
        {
            ...
            if( dtrm <= std::numeric_limits<double>::epsilon() )	//==0
            {
